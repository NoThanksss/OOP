...
namespace TicketSalling2._0
{
    ...
    public class Flight
    {
        ...
        */
        ...
        public Seat Reserv(Passenger passenger, SeatType type, bool atWindow)
        {
            ...
            for (int i = 0; i < airplane.numberOfRows; i++)
            {
                for (int j = 0; j < airplane.numberOfColumns; j++)
                {
                    if (atWindow) 
                    {
                        if (type.Equals(seatsArr[i][j].type))
                        {
                            if (seatsArr[i][j].atWindow)
                            {
                                seatsArr[i][j].ReservSeat(passenger, type);
                                return seatsArr[i][j];
                            }
                        }
                    }
